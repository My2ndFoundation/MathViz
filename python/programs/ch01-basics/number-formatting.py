...
# <<< BLANK
# >>> BLANK id=receipt-specs level=3 hint="交回一整行收据，写成一个双引号 f-string，四列依次是：name 靠左占 12 列；quantity 靠右占 4 列；price 靠右占 9 列、两位小数；total 靠右占 12 列、带千位分隔、两位小数。四列的对齐符都明写，数字默认靠右也照写 || 两位小数用定点类型 f；四个 {变量:规格} 首尾紧挨，中间不夹任何字符 || 每个规格内部按讲解里那个固定语序排：对齐、宽度、逗号、精度、类型" hintEn="Hand back one whole receipt line as an f-string in double quotes, four columns in this order: name on the left in 12 columns; quantity on the right in 4; price on the right in 9 with two decimal places; total on the right in 12 with thousands separators and two decimal places. Write the alignment out for all four, even where a number would sit on the right by default || the two decimal places use the fixed-point type f; the four {variable:spec} fields sit back to back with nothing between them || inside each spec keep the fixed word order from the notes: align, width, comma, precision, type"
