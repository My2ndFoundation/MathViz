...
## 只读
不改 worktree、索引、HEAD、分支。临时文件放 {{REQUIRED scratchpad}}，前缀 `{{波名}}-review-`。改动性检查串行做、从内存原字节复原。`check.py` 至多跑一次。你不派子代理。
...
- 中英两种语言是否等义？学生从程序与讲解能否推断出挖空行里的字面量文字？

**门与测量**：每个 P 参照的机制是否与被测程序不同；构建者报告的变异是否真能让门变红（抽一个自己跑）。
