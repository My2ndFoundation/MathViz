...
(function (root, factory) {
  ...
  function mount(el, cur, opts) {
    ...
    /* relabel() 专用：切语言时把 OUTPUT 空态占位重画成当前语言。
       旧账：paintOut() 是增量渲染（见上面的注释），空态占位只在
       「outLen===0 且 children.length===0」这个条件成立时 appendChild
       一次；一旦占位已经画上，语言切换不会让 outLen/children.length 变化，
       这个条件就再也不成立——占位因此永远停在建面板那一刻的语言，
       正向反向都一样（阶段 8 Task 9 查证过）。
       只在「当前确实是空态」时动手：outLen===0 时 paintOut() 维持的
       不变量是 list 里恰好一个孩子、且就是这个占位——有输出时 list 装的
       是真日志（outLen>0），绝不能被占位覆盖。 */
    function relabelEmptyOut() {
      if (outLen === 0 && paneOut.list.firstChild) {
        paneOut.list.firstChild.textContent = T(DBG_UI.noOut);
      }
    }
