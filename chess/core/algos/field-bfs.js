/* 最少步数场（多起点广度优先搜索）的**算法源码生成器**（规格 §2.1）。
   ...
   **一个字都没动的**：`DX` / `DY` 八个方向、`onBoard`、`dist` 那张表与它的
   ...
   · **源格自己的距离是 0，而且 0 要真的从 `mark` 通道以数值送出去**
     ...
     0 画成贴地的一根零长柱 + 一个 `"0"` 标签，与「没有柱子」长得不一样 ——
     **柱顶那张网格在没有柱子的格子上是断开的**（`drawFieldMesh` 的 `topOf`
     返回 `null`），不会悄悄贴地连成 0。两头合起来这条裁定才成立。
     ⚠ 初稿这里写的是「与从没标过的**暗灰小点**长得不一样」—— **工具⑤ 里没有
     那个小点**：工具① 的 `drawHeightField` 遍历每一格，所以「没有值」是个真分支；
     工具⑤ 的 `drawMarks` 遍历的是 `c.marks`，没标记的格子根本不进循环。
     硬补一个暗灰点反而会把回放途中「还没铺到」的格子说成「到不了」（9f 第十四处）。
     算法这一头由种子那个 `mark(s, 0)` 负责，
     `field-bfs.test.js` 用打桩 host 钉住它收到的是**数值 0**。
