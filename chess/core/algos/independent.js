/* 马的最大独立集（回溯 + 两条剪枝）的**算法源码生成器**（规格 §2.1）。
   ...
   **生成出来的那段源码里的注释和命名，是写给一个没学过棋、正在学算法的
   ...
   ④ **`cut` 这种 mark 计划的骨架根本没发**。规格 §4⑤⑤ 明写「上界剪枝这一条
      ...
      每次剪枝恰好 +1 步，N=5 从 191,021 涨到 **193,200**（下面实测表里那个数）。
      ⚠ 这一行初稿写的是「涨到 193,196」，**少算了 4 步**：`191,021 + 2,175`
      只加了剪枝，**忘了这一档还有 4 次 `log`**（新纪录时才发一次）。
      `191,021 + 2,175 + 4 = 193,200` 才对得上。记在这里是因为它正是本阶段
      反复出现的那一种错 —— **算式自洽，但少数了一项**，而自洽最容易骗过复核。
