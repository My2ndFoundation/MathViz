/* 带障碍盘上「最少的王支配所有空格」的**精确**算法源码生成器（规格 §2.1）。
   ...
   **它的精确解在解释器里跑不完**。简报自己预警过这一条。
   这不是外推出来的：把 `Interp.run` 的 `opts.limit` 抬到五千万，**实测那一档
   要 251,668 步**（k=5，未截断）—— 比 200,000 的上限高出 26%。在默认上限下
   它就是 `truncated === true`、`result === undefined`。留着它，四档里就有
   两档撞墙，「差距那一课」只剩一档在演。

   ⚠ **「解释器步数 ÷ 算法操作数」有两套口径，千万别混用**（这里一度混过）：

     档                      简报口径 ops   本仓库口径 ops   解释器步数    倍率(简报/本仓库)
     5×5 [6,12,18]              1,440           542          18,174     12.6× / 33.5×
     6×6 [6,24,35]              6,952         2,372          71,742     10.3× / 30.2×
     6×6 七墙（原第三档）       24,779         9,971         251,668     10.2× / 25.2×
     6×6 [1,4,24]（新第三档）      ——          1,309          41,627       ——  / 31.8×

   两套差 2.5–2.7 倍，因为「一次操作」的定义不同（简报那套数得细）。
   **拿简报表里的 ops 预估新档步数，要乘 ~10–13，不是 ~30。**
   本文件表里的四档步数一律是直接量的，不是任何一种乘出来的。
...
(function (root, factory) {
  ...
  const TAIL = [
    ...
    '  limit = limit + 1;',
    {
      ...
      en: [
        '  log("Trying " + limit + " king(s) — is that enough?");',
      ],
    },
