/* 马的攻击格（位盘 · 一次挪整块盘）的**算法源码生成器**（规格 §2.1）。
   ...
   **生成出来的那段源码里的注释和命名，是写给一个没学过棋、正在学算法的
   ...
   ② **四张掩码的十六进制常量**：打印成二进制数过——`NOT_A`/`NOT_H` 各是
      ...
      线），且四个常量的最高字节（对应第 56–63 位，也就是第 8 行）都不是
      全 0，所以这四个 BigInt 字面量本身就是「恰好 64 位宽、最高位是 1」，
      不会在第 63 位以上悄悄带一串隐含的 1。

   ③ **`b & FULL` 这一步是否必须**：**不是必须的，是防御性的**。②已经
      证实四张掩码本身就恰好是 64 位宽（最高字节非零），跟它们 `&` 之后，
   ...
   · **宿主桥接只用一个**：`mark(sq, "ok")`——这道题不需要 `place` 摆真正
     的棋子（马本身在源码里只是一个位，不通过宿主桥接摆），也不需要
     `attacked` 反查、不需要 `clear`、不需要 `log`。只调 `mark` 不代表桥接
