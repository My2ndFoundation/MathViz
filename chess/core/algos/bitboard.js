/* 马的攻击格（位盘 · 一次挪整块盘）的**算法源码生成器**（规格 §2.1）。
   ...
   **生成出来的那段源码里的注释和命名，是写给一个没学过棋、正在学算法的
   ...
   ② **四张掩码的十六进制常量**：打印成二进制数过——`NOT_A`/`NOT_H` 各是
      ...
      值必然小于 2^64**——不是「最高位是 1」：`NOT_A` 位长确实是 64，但
      `NOT_H`（`0x7f7f…`）是 **63**、`NOT_AB`/`NOT_GH`（`0xfcfc…`/`0x3f3f…`）
      是 **62**，后三张的最高位就是 0。（初稿在这里写的是「四个常量最高字节
      非零，所以恰好 64 位宽、最高位是 1」——结论对，理由错，而且那个理由
      本身也推不出结论。复审复验时抓出来订正。）
   ...
   · **本任务只调一个桥接**：`mark(sq, "ok")`。⚠ **「要不要 `place(SQ, "wN")`
     摆一枚真的马」是一个未决问题，由 Task 4 裁定，不是这里结的案**——规格
     §4⑤① 的原话是「摆**真的马**（`place(sq, 'wN')`）」，而计划把这一条列成
     Task 4 的待判项（摆了就多一次宿主调用、多一行源码，值不值要那时候
     判）。**若 Task 4 判定要摆，这个生成器得回头加一行**。这道题另外三个
