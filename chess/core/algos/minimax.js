/* 极小极大 / α-β 的**算法源码生成器**（规格 §2.1）。
   ...
   **生成出来的那段源码里的注释和命名，是写给一个没学过棋的 16 岁读者的**
   ...
   子集约束（规格 §2.6 的解释器只认这么多语法），踩过的坑记在这里：
     ...
     · `applyMove` 用 **make/unmake**（就地改再改回），不是整盘拷贝。这不是
       风格问题：实测整盘拷贝下 α-β 深度 4 要 245,847 步，超过
       STEP_LIMIT=200000 直接截断；make/unmake 只要 119,587 步。**差一整层深度。**
...
(function (root, factory) {
  ...
  /* 起始局面登记处。`sq = rank * 4 + file`，正=白 负=黑，`1=P 2=N 4=R 5=K`。
     ...
     这个工具的课就上不成 ——
       1. 深度 2/3/4 三个 tab 的答案彼此不同（0 / 112 / −12），否则「多想
          一层会改主意」这句话没有证据；
       2. 深度 3 三种模式都跑得完（62,122 / 37,955 / 26,094 步），能完整
          展示剪枝；
       3. 深度 4 纯 minimax 撞上 STEP_LIMIT 而 α-β 跑得完（392,663 vs
          119,587）—— 规格 §4④ 的那一课不是「慢一点」，是「做不到」。
  ...
  const HEADER = [
    ...
    '     1 = 兵(P)   2 = 马(N)   4 = 车(R)   5 = 王(K)',
    '   分数永远站在白方视角：越大白方越好，越小黑方越好。',
    '   于是白方在自己的回合挑最大的，黑方挑最小的 —— 这就是「极小极大」。 */',
    '',
    'const N = 4;',
    '',
    '/* 每种棋子值多少分，下标就是上面那个棋子编号。这张表就是这个 AI 的',
    '   全部价值观，改一个数字它就会下出不一样的棋。',
    '   注意王（下标 5）现在是 0 分 —— 这份搜索没有「将军 / 将死」的概念，',
    '   王只是一个走得慢的子。把它改成 10000 再跑一遍，看答案变不变。 */',
    'const VAL = [0, 100, 300, 0, 500, 0, 10000];',
    '',
    ...
    '   靠它们判断哪些分支根本不必再看。两个 tab 的差别只有循环里那两行。 */',
  ...
  const LOOP_HEAD = [
    ...
    '    // unmake：原样还回去。也可以整盘复制一份来走，但那会慢一整层深度。',
    '    bd[from] = bd[to];',
    '    bd[to] = cap;',
    '    if (white) {',
    '      // 白方回合：谁大留谁。',
