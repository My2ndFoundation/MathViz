/* 带障碍盘上的**二分图最大匹配**（行段 × 列段）的算法源码生成器（规格 §2.1）。
   ...
   **后半句不成立**，而且不是边角情形 —— 简报自己那四档盘里有三档都是反例：

     5×5 `[3,10,13,17,18]`   最大匹配 6，**4 辆车就盖满了**（0, 9, 11, 20）
   ...
   **5 / 6 / 7 / 9 与简报表逐格对上**（rook-cover.test.js 拿一份独立的匈牙利实现对拍）。
   ...
   与 `result`；`mark` 次数与最深调用栈由 rook-cover.test.js 的 `probe()` 量：
     ...
     5×5 [3,10,13,17,18]               6        903         2        28  (18/6/4/0)
   ...
   **最大匹配数四档与简报表逐格相同**（5 / 6 / 7 / 9），那才是答案。
   深度与访问数是实现形态的后果，测出来是多少就报多少（照 `knight-path.js`
   文件头同一条规矩）：简报的「增广路最深 1 / 5 / 4 / 7」本文件量出来是
   1 / 2 / 4 / 7 —— 四档里三档逐格对上，只有第二档不同。
   **第二档（5×5 五障碍）在这一份实现下退化了**：`back = 0`，六辆车全是第一趟
   贪心直接坐下的，一次成功的增广都没发生（深度 2 是一次**失败**的尝试）。
   它原本要教的「匹配第一次真的做事」因此落空。若要保住那个教学点，
   5×5 五障碍里有的是替代档，例如 `[3, 14, 17, 18, 21]`（行段/列段 8/8、k=7、
   back=2、最深 6 层、1,156 步）或 `[2, 5, 6, 13, 22]`（8/8、k=7、back=2、
   最深 7 层）—— 换不换是页面（Task 3）与规格的事，本文件不替谁做主。

   离上限差两个数量级：这一题**不存在撞墙风险**。四十块 3..6 边长的随机盘
   （固定种子，见测试）里最贵的一块也只有约 2,000 步。 */
