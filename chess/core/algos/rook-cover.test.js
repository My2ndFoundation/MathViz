...
/* 四档盘。第一、三、四档是简报给的，一个数没动。

   **第二档换过一次**（修复轮 1，控制器裁定）：简报原本给的是
   `[3, 10, 13, 17, 18]`，它在这份实现下**退化了** —— 六辆车全是第一趟贪心
   直接坐下的，`back = 0`，一次成功的增广都没发生，而这一档要教的正是
   「匹配第一次真的做事」。换成 `[3, 14, 17, 18, 21]`：行段/列段仍是 8/8、
   k = 7 < 8、`back = 2`、增广路最深 6 层 —— 挪窝真的发生了，看得见。
   （标签仍是「五障碍」：五个障碍。简报把它写成「四障碍」，数错了一个。） */
const BOARDS = [
  ['5×5 空盘',   5, 5, [],                                5],
  ['5×5 五障碍', 5, 5, [3, 14, 17, 18, 21],               7],
...
/* ---- 有障碍的那三档：增广路真的走起来了 ----
   这几条是上面那组空盘断言的对手戏。没有它们，一份「永远只做第一趟贪心、
   从不递归」的实现照样能通过空盘那六条 —— 它只会在有障碍的盘上少数几辆车。

   **第二档也在这里**，而且它在这里是有来历的：简报原来那一档（`[3,10,13,17,18]`）
   `back = 0`，整档退化成纯贪心。这条断言就是那次退化的守卫 —— 谁再把第二档
   换回一块「贪心就能解完」的盘，这里当场红。 */
{
  const three = [
    ['5×5 五障碍', 5, 5, [3, 14, 17, 18, 21]],
    ['6×6 九障碍', 6, 6, [0, 6, 8, 9, 14, 20, 24, 26, 27]],
    ['6×7 九障碍', 6, 7, [4, 6, 7, 8, 26, 31, 33, 35, 37]],
  ];
  let deepChecked = 0;
  for (const [label, W, H, blocked] of three) {
    const p = probe(W, H, blocked);
    T.ok(p.kinds.back > 0, label + '：确实有车被请下来挪过窝（back ' + p.kinds.back + ' 次）' +
         '——这一档不是贪心就能解完的');
    T.ok(p.depth > 1, label + '：增广路不止一层（最深 ' + p.depth + '）');
    deepChecked++;
  }
  T.eq(deepChecked, 3, '三档有障碍的盘都验过增广路真的走起来了');
}
