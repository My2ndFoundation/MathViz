...
/* ================= 四档盘 =================
   ...
   跑不完**：把 `Interp.run` 的 `opts.limit` 抬到五千万直接量，那一档要
   **251,668 步**（k=5、未截断），比 200,000 的上限高出 26%；在默认上限下它就是
   `truncated === true`、`result` 变成 `undefined`。四档里有两档撞墙，「差距那一课」
   就只剩一档在演了。
   ⚠ **别拿「算法操作数 × 倍率」去估这个步数。** `king-exact.js` 文件头那张对照表
   记着两套 ops 口径（简报口径 24,779 vs 本仓库口径 9,971，同一档），两套差 2.5–2.7
   倍 —— 混着用会算错一倍有余。本文件表里的四档步数一律是**直接量的**。
