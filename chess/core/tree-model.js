...
(function (root, factory) {
  ...
  /* ---------------- 从 varDelta 里读局部量 ----------------
     ...
     由此：
       mvCount = 出帧步里 ms 的 from 的长度（depth===0 那种当场返回的帧根本
                 没走到 `let ms`，出帧步里就没有它 → 0，这是真话不是兜底）
  ...
  /* build(trace, entryName) → tree
     ...
     Node = { id, parentId, depth, childIds, pushStep, popStep,
       ...
       popStep        出帧步下标；**-1 表示这一帧从未关闭**——轨迹撞上
                      STEP_LIMIT 被截断时（α-β 那个 tab 的深度 4 纯 minimax
                      就会）栈上所有帧都停在开着的状态，这是设计内的情形，
                      不是异常。此时 mvCount/value 也只能是 0/null。
