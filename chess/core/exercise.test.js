...
T.throws(function () {
  parseZh([
   ...
   /* pattern 前半段 `BLANK 指令里有一段认不出来的内容` 是 assertKnownTokens
      自己写的那句「这是哪道检查」的文案，后半段 `"你好的场景""` 是被回显的
      那个没人认领的 token。**必须含前半段**：只锚回显值的 pattern 什么都
      证明不了——把这道守卫的措辞整句换掉、回显值原样保留，测试照样全绿
      （实测过），于是「是这道守卫响的」根本没被断言。回显值留着是额外的
      区分度（把这条跟下面那条 tip= 的分开），不是判据的全部。 */
   'BLANK 指令里有一段认不出来的内容 "你好的场景""');
...
T.throws(function () {
  parseZh([
   ...
   /* 同上：锚的是守卫自己那句 `BLANK 指令里有一段认不出来的内容`，回显出来的
      `tip="乙"` 只是附加的区分度。这两条是 assertKnownTokens 仅有的两条断言，
      两条都只锚回显值的话，那道守卫等于没有测试。 */
   'BLANK 指令里有一段认不出来的内容 "tip="乙""');
