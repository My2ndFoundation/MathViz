...
# 侧栏/页眉里放可见 logo 的导航页（根目录两页 + 每个子项目两页）。工具页只拿
# favicon——它们各有自己的面板与标题，塞一个品牌标记进去只会跟工具自己的图例抢注意力。
#
# ⚠ 这行注释里**故意不写个数**。它写过「六个」，加 python 子项目那次这个数就变
# 陈旧了——下面的清单长了两条，散文没跟上。这是本仓反复记的同一族事故（根
# index.html 的卡片文案里「两个工具」漏改成「三个」又漏改成「五个」；62 条里
# 48 条版本号静默漂移）。**清单本身就是那个数**，脚本运行时也会打印真实条数
# （「其中 N 个另带可见 logo」）；再抄一份进散文只是多给自己一次漂移的机会。
BRAND_PAGES = (
    'index.html', 'app.html',
