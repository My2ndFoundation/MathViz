...
**第 4、5 章把这条钉死了。** 两章共派出 13 个 agent，**5 个报回了 brief 的事实错误，
全部成立**（另有 1 个按 brief 的指示挖出了共享 core 的真 bug）。下表把全程被顶回来的
都列在一起——**前五行是那五条 brief 事实错误**，第六行是那个 core bug，最后一行来自
第 3 章的上一版交接文件：
...
| `fromBytes` 报错里的那个字节值 | `0xFA` 是**种子相关**的，不是常量；类别可复现，字节值不可 |
| `modInverse` 的取值范围（brief 没写错，是它照着查出来的） | `mod()` 在 n > 2⁵² 时静默差 1 —— 一个真的 core bug |
| 「39/56，失手全落在约数上」（来自上一版交接文件） | 三方三个数，且"是否落在约数上"三方结论相反 |
