...
/* ⚠ **计划订正（2026-08-08，阶段收尾）**：下面这两条的「钉住现状、等裁定」
   已经被裁定收掉了 —— 结论写在规格 §7.7「探针跑完的裁定（2026-08-08）」。
   ① 五个复合赋值与 `>>>` **不进子集，但各要一句点名符号的 `unsupported`
   诊断**（本计划漏了这个「第三个选项」，于是 Task 3 删掉 UNSUPPORTED_BINOP
   五条之后，这六种写法静默降级成了 `syntax`）；② `toString(2)` 不加。
   `interp.test.js` 里这两条的实际形状已按裁定改过，**以文件为准，不以这里
   为准**。留着这一段是因为它记录了当时为什么先钉后裁。 */
