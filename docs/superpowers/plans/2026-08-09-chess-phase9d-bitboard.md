...
**Architecture:** 入口 `addDir(k)` **按方向递归**（末尾调 `addDir(k + 1)`），于是调用深度 = 第几个方向，**深度塔九层**（`addDir(0)`–`addDir(8)`，末一层是 `k >= 8` 的终止帧；⚠ 不是八层，2026-08-10 实测订正），第三根轴不需要任何额外声明（与 9c 的 `fillLayer(d)`、`knightPath` 的 `expand(frontier, d)` 同构）。掩码写在源码里、是完整正确的一份；`tips` 叫她**亲手删掉再 Run 一次**，看盘上冒出来的幽灵格 —— 零额外机制，源码不带人造开关。
...
**期望：仍然全绿。** 这不是失败 —— 要你**亲眼看到**：结果正确性测不出「递归还是循环」，而第三根轴（九层塔，见 Task 5 Step 2 第 2 条的订正）完全依赖它。**这一条只能靠 Task 5 的浏览器验收守，报告里要写明。**（9c 的同一个缺口实测过一次，结论相同。）验完还原。
...
```js
    bitboard: {
      ...
      /* 入口是**递归**那一个：addDir(k) 处理第 k 个方向、末尾调 addDir(k + 1)。
         于是调用深度 = 第几个方向，深度塔九层（八个方向 + 一个终止帧），第三根轴
...
2. **⚠ 那个实验，要说到能照着做** —— 规格 §4⑤② 的原话是「`tips` 里必须点名到『删哪几行』而不是含糊的『删掉掩码』，她要能照着做」。所以：**把滑杆拖到 a 线上的某一格**（比如 a4），**删掉源码里 `& NOT_H` 与 `& NOT_GH` 那几处**，Run 一次，**看 h 线上冒出来的幽灵格**。

   ⚠⚠ **本计划初稿这里写的是「删 `& NOT_A` 与 `& NOT_AB`」—— 那是错的，而且是静默错的**（2026-08-10 实测订正，本阶段第七处）。实跑：

   ```
   a4 正确答案            : b2 c3 c5 b6
   删 NOT_A + NOT_AB  →  b2 c3 c5 b6      幽灵格：（一个都没有）
   删 NOT_H + NOT_GH  →  h1 b2 g2 c3 g4 c5 h5 b6   幽灵格：h1 g2 g4 h5
   ```

   **照初稿的 tips 做，她什么都看不见** —— 这道题唯一的那一课当场蒸发，而且不报任何错（正是这道题在教的那种失败）。

   **原因值得写进 `tips` 本身**：**掩码写的是「落点」，不是「出发点」。** 掩码是在移位**之后**才 `&` 上去的，所以它挡的是**位落在哪条线**。a 线上的马往左移会**落到 h 线**，因此拦住它的是 `NOT_H` / `NOT_GH`；`NOT_A` / `NOT_AB` 拦的是**从 h 线往右绕回 a 线**的那一半。

   对称的另一半（可以写进 `tips` 当第二个实验，也可以只留一个）：

   ```
   h5 正确答案            : g3 f4 f6 g7
   删 NOT_A + NOT_AB  →  幽灵格：a4 b5 b7 a8
   删 NOT_H + NOT_GH  →  幽灵格：（一个都没有）
   ```

   ⚠ **写完 `tips` 自己按它做一遍**（node 里模拟即可），确认幽灵格真的出现。**这一条是本任务唯一不能只靠读代码确认的东西。**
3. **按 `3` 切侧视** —— 一层一个方向；而删掉掩码之后，**错落在具体某一层上**，一眼看出是哪个方向漏了。
...
**这一条是 Task 1 Step 8 那个缺口唯一的守卫**（结果正确性测不出「递归还是循环」，而九层塔完全依赖它）。**不改任何代码。**
...
2. **⚠ 深度塔九层** —— 按 `3` 切侧视。**塔必须是九层，不是一层。** 若是平的，说明 `addDir` 被写成了循环 —— **这是本任务最重要的一条，报告里要点名**（调用栈面板若能看到 `addDir()` 逐层嵌套，那是更硬的证据，一并记）。

   ⚠ **是九不是八**（2026-08-10 Task 1 复审实测订正，本计划初稿这里写的是「八层」）：`addDir(k + 1)` 是**无条件**尾调，`addDir(8)` 真的被调用、真的压一帧，进去才被 `if (k >= 8) { return; }` 弹回。`TreeModel.build(trace, 'addDir')` 实测 **9 个节点、depth 0..8、每层 1 个**（sq = 0 / 27 / 63 三格同结果）；换成外层 `for` 循环则是 **8 个节点全在 depth 0**。**照初稿的「必须是八层」去验，一份完全正确的实现会被判不通过** —— 判别点是**九 vs 一**，不是八 vs 九。数不清第九层薄不薄没关系，**塌成一层才是要拦的**。
