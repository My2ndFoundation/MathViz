...
- **版本 patch 递增 `1.1.0` → `1.1.1`**，51 个工具全改。三处落地：HTML `<meta name="tool-version">`、HTML 头注释 changelog 顶行、`tools.json`。**changelog 标题两种写法（`版本记录（新→旧）：` / `版本记录（changelog，新→旧）：`）保持各文件原样。**
...
meta 的 patch 位加一（`1.1.0` → `1.1.1`）。头注释 changelog 最上方加一行：
