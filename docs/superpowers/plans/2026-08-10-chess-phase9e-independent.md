...
⚠ **上表是开工前探针的数**（骨架还没有 `cut` 的 mark 与 `log`）。**最终生成器**实测 **2,593 / 18,285 / 193,200**（96.6%）—— **引用步数一律用这一组**；「塔层数」那一列是**调用栈深度**，画出来的塔是 `SQ` 层（9 / 16 / 25）。
| 6 | — | 撞上限 | — | — | — |
...
> **【2026-08-10 补记：Task 1 跑完的实测值】** 上面三个是**加 `cut` 之前**的基准，Task 1 逐字复跑确认一字不差。**最终生成器**（补了 `cut` 与 `log`）的实测是：
>
> ```
> N=3  2,593 步   N=4  18,285 步   N=5  193,200 步（96.6% 上限，truncated: false）
> ```
>
> **滑杆上限定 5。** 后续任务凡是要引用步数（`tips` 文案、changelog），**用这一组，不要用上面那组基准** —— 两组数打过一次架（Task 3 撞见）。
...
```js
    independent: {
      ...
      /* ⚠ 塔最深 25 层（N=5，= SQ；调用栈深度是 26 = SQ+1，两个数别混 ——
         Task 4 实测订正，见规格 §4⑤ 9e ④）。26 × 0.20 = 5.2 世界单位（按 26
         配，多留一层，方向安全），与 queens 的
...
2. **⚠ 深度塔立起来了，而且没有长过头** —— 按 `3` 切侧视。塔**一层一格**：⚠ **画出来的塔 = `SQ` 层**（N=4 → 16、N=5 → 25），而**调用栈深度 = `SQ+1`**（17 / 26，调试面板上那个「深度」写的是它）—— **两个数别混，别照「SQ+1」去数塔**（9d 就因为差一层的层数差点把正确实现判为不通过）。⚠ **重点看取景框有没有把棋盘缩没** —— 这是 `spanZ: 0.20` 唯一的验收点，**若塔顶出画或棋盘缩成一点，报出来并给出你量到的合适值**。
3. **N=3 → 5 三档都跑得完**（状态牌不出现 `truncated`），**读数分别是 5 / 8 / 13**。⚠ **N=5 是 96% 上限，特别看它有没有撞**。
4. **滑杆拖到 5 时步数 = 193,200** —— 与 `tips` 里写的数**逐字**对得上（三档 2,593 / 18,285 / 193,200）。⚠ 别用 191,021，那是加 `cut` 之前的基准。
