...
> **⚠ 2026-08-07 订正（Task 6）：上表第三行的档在交付时换掉了。** `6×6 七障碍` 换成了
> **`6×6 [1,4,24]`（贪心 8 / 精确 4，差距 4）**——它比差距 2 锋利，而且与上一档
> 同尺寸、同墙数、只挪了位置，两档并看就是「贪心吃多大亏取决于墙摆在哪儿，不是
> 摆了几堵」。`rookCover` 的第二档同样换过（换成 `5×5 [3,14,17,18,21]`，见 Task 2
> 的修复轮）。**两题最终交付的四档、以及解释器步数，见规格 §4⑤ 的两张表**（那两张
> 表是 Task 6 在浏览器里逐格复验过的，与本表的「算法操作数」不是同一个量）。
...
**⚠ 「最深递归」不等于「增广路最长」（2026-08-06 Task 3 实测订正，修复轮 1 再订正一次）。** 上表末列那几个深度是 `walk` 的最深递归，逐档拆开是：

| 档 | 最深递归 | 其中最深的**失败** | 其中最深的**成功** |
|---|---|---|---|
| 5×5 空盘 | 1 | **0**（一次失败都没有）| 1 |
| 5×5 障碍 | 6 | 6 | 3 |
| 6×6 障碍 | 4 | 4 | 2 |
| 6×7 障碍 | 7 | 7 | 2 |

所以**后三档**最高的那座塔都属于一次**失败**的搜索，真正走通的增广路浅得多；**第一档是例外**——空盘上一次失败都没有，那 1 层是一次成功。（初版订正写成「四档里每一档都……」，在默认档上当场是假的，修复轮 1 的审查抓到。）

原先在第四档写的「增广路最长的一档——横竖交替走七段」是错的：那 7 层里一辆车都没多。这不是缺点，是这一档最好的一句话——**正是那几趟失败的搜索问过的段，圈定了 König 的那些线**。
...
**2026-08-07 交付时对齐过一次**（Task 6）。计划里没有的两件事也记在这里，因为它们真的做了：

- **中途插入了 Task 3A（`board(st)` 多交一样东西 —— 墙）。** 原计划让两道新题各自去画障碍；实际做法是把 `board(st)` 的返回值从 `{ files, ranks }` 扩成 `{ files, ranks, blocked? }`，走 `BoardRender` 现成的 `spec.mask` 再补一层斜纹。**`core/board-render.js` 一行未动**，两道新题因此各自只多 return 一样东西。规格 §4⑤ 的 `PROBLEMS` 草图已经照这个改了。
- **Task 6 立了三道 `validateDecl` 门**（计划里只有「定版 + 全量门 + 验收」）。三道门守的都是**这一阶段自己踩过、而且只有靠人眼看截图才发现**的坑：多盘题漏声明 `sideView`、声明层文案过长挤坏版面、两条 `role:'edit'` 轨道绕过 localStorage 那条限制。见规格 §4⑤「声明层有三条硬约束」。

- [x] 七个测试文件全绿；`check.py` exit 0 七道门；`sync_registry --check` exit 0
- [x] `rookCover` 四档的最大匹配与宿主侧匈牙利对拍一致，且**摆出来的车经独立验证真的覆盖了所有空格**（覆盖是真的，但**不是最少的车数**——见订正）。四档 k = 5 / 7 / 7 / 9，浏览器里另验了 König「行段数 + 列段数 = k」逐档成立
- [x] `kingDominate` 四档：至少一档 **贪心 > 精确**（实际是三档：0 / 2 / 4 / 不知道），且 **7×7 那一档精确解撞墙**并显示「跑不完，判不了」（琥珀，`result` 是 `null`、`truncated` 是 `true`，页面没有任何一处说 8 是最优）
- [x] 两题**各自只动了三处**（`git diff --stat` 为证）
- [x] `T.throws` 认消息；「插入指令行不改变步数」有常驻门，两者都经突变确认有牙
- [x] 阶段 5/6 的行为一字不差（五题里前三题、练习模式、挖空全部照旧；Task 5 拿 HEAD 与工作区两份页面对跑 17 组，逐项相同）
- [x] 版本落三处（含 `VERSION` 常量），角标 `v1.2.0`
- [x] **（Task 6 追加）**两道新门各自经变异确认会拦；页签条加到五个之后两头各切掉一块的问题一并修了（见 1.2.0 changelog）

**下一阶段**：阶段 8 扩展题目（`bitboard` / `pathCount` / `independent` / `fieldBFS`），以及给这两道新题补挖空（§8 明写这一阶段的产出质量应在实际用过一轮之后复盘）。

**阶段 8 开工前的第一件事**（Task 6 留下的）：把「声明层文案过长挤坏版面」这道门从**字数近似**推进到**真的量版面**。今天那道门量的是声明层唯一量得出来的东西（显示宽度上限 72 / 26），而两次坏掉的都是运行期几何；运行期判据不能直接立（窄窗口下图例本来就 `display:none`，一条「图例右缘要在面板左缘左边」的断言会在合法的窄窗口上误报）。可行的方向是把判据挪进那次已经在做布局的地方（`layoutTracks` / `paintLegend` 之后）并且**只在图例真的可见时**才判。
