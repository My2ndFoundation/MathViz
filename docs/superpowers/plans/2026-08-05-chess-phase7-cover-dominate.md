...
**Goal:** 给工具⑤ 补上最后两道题——带障碍盘上的**最少车覆盖**（二分图匹配 + König 定理）与**最少王支配**（贪心与精确同屏并排），并让「最优解你等不起」这件事在屏幕上可见。
...
**题面**：带障碍盘上用最少的车覆盖所有空格。行被障碍断成若干**行段**、列断成若干**列段**；一个空格同时属于一个行段与一个列段——那就是二分图的一条边。最大匹配 = 最少的车数（König：最大匹配 = 最小点覆盖）。
...
for (const [label, W, H, blocked, expectK] of BOARDS) {
  ...
  T.eq(r.result, hostMaxMatch(W, H, blocked), label + ' 的最少车数与宿主侧匈牙利一致');
  T.eq(r.result, expectK, label + ' 的最少车数是设计时量的那个数');
}
...
| 档 | 行段 / 列段 | 空格 | 最少车 | 增广路最深 | 算法访问数 | 这一段教什么 |
|---|---|---|---|---|---|---|
| 5×5 空盘 | 5 / 5 | 25 | **5 = 行数** | 1 | 35 | **匹配根本没上场**——每行一个就够，这就是为什么这道题必须有障碍 |
| 5×5 `[3,10,13,17,18]` | 8 / 8 | 20 | **6**（比两边少 2）| 5 | 27 | 匹配第一次真的做事 |
| 6×6 `[0,6,8,9,14,20,24,26,27]` | 10 / 10 | 27 | **7**（少 3）| 4 | 30 | 差距更大 |
| 6×7 `[4,6,7,8,26,31,33,35,37]` | 12 / 12 | 33 | **9**（少 3）| **7** | 72 | **增广路最长的一档**——横竖交替走七段，肉眼最看得清 |

算法访问数只有 27–121，比 `kingDominate` 小三个数量级，**`rookCover` 这边不存在撞墙风险**。第一档（空盘）是有意留的对照：它让「为什么要有障碍」这件事不用讲、看一眼就知道。
...
四档的**算法访问数**已在上表给定；**解释器步数要你自己量**（它取决于你怎么写）。四档都应当远在 200,000 以内。把实测表贴进报告，并与上表的最少车数逐格对照——**任何一格对不上就停下来报告**，那说明行段/列段的建法与参照实现不一致。
...
- [ ] `rookCover` 四档的车数与宿主侧匈牙利对拍一致，且**摆出来的车经独立验证真的覆盖了所有空格**
