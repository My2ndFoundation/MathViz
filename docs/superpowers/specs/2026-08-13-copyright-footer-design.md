...
| 导航壳（`app.html` ×3） | 3 | **不放版权**，只留 `.sb-cookie` 槽给「Cookie 设置」 | — |
...
初版给三个壳的侧栏也铺了完整串。**这是错的**：壳是容器，它的 iframe 里装的要么是
画廊（页脚有版权）、要么是工具（面板底部有版权），所以壳自己再放一份，任何时刻
屏幕上都会同时出现两条。线上实测两种状态各自都是 2 条。

根因是把 6 个导航页当成平级的，而其中 3 个壳**包含**另外 3 个画廊。同一个结构
盲点在 GA 那边被抓住了（iframe 双计 pageview、叠两个同意横幅），在版权这边漏了。

壳里保留一个 `.sb-cookie` 窄槽，只承载「Cookie 设置」——那个链接不重复，因为 GA
代码块在 iframe 内提前 `return`（实测壳里 1 个、iframe 里 0 个）。槽为空时
`:empty` 让它自己消失。
