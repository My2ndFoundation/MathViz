...
| 滑块展示总次数 | 1240 |
| **其中对当前页签完全无效** | **67%** |
| 一个滑块都不起作用的页签 | 11（5%，旧探针读数；剔除假阳性后只会更少） |
| **无时间驱动（静止）的页签** | **72（39%）** |
| 全部页签都静止的工具 | 4 |

上表的滑块有效性一列与下列各页签的「有效」计数，来自阶段 1 交付的 `scripts/audit-scenes.html`（补上了 GPU 读回 flush 与逐场景管线预热，覆盖 188 个页签），取代早期手写探针的读数。

最严重的十个页签：

```
complex-mult-3d      demoivre   16 个滑块 → 有效  1
