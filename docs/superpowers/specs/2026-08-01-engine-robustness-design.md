...
51 个工具 **各自 patch +1**。三处落地：HTML meta、HTML 头注释 changelog、`tools.json`。changelog 标题两种写法保持各文件原样。

**不要写死 `1.1.0` → `1.1.1`。** 实测版本号是分散的：`1.1.0` 35 个、`1.0.1` 9 个、`1.2.0` 3 个、`1.1.1` 3 个、`1.3.0` 1 个（`complex-mult-3d` 就是那个 `1.3.0`）。规则是**读该文件当前的 meta，patch 位加一**——`1.0.1` → `1.0.2`、`1.3.0` → `1.3.1`，以此类推。照字面套 `1.1.0` → `1.1.1` 会给 **16 个文件**写错版本号。

`trig-essence-3d-new` 也在其中（当前 `1.1.0`，changelog 标题写法为「版本记录（新→旧）：」）。
